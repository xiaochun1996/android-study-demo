...
class DebugToolDialogFragment : AppCompatDialogFragment() {
    ...
    override fun onCreateView(
        ...
        val parent = window?.findViewById(android.R.id.content) ?: container
        val view = inflater.inflate(R.layout.debug_tool, parent, false)

        window?.setLayout(
            DisplayUtil.getDisplayWidthInPx(view.context) * 0.7f.toInt(),
            WindowManager.LayoutParams.WRAP_CONTENT
        )
        window?.setBackgroundDrawableResource(R.drawable.shape_hi_debug_tool)
    ...
    inner class DebugToolAdapter(val list: List<DebugFunction>) :
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
            val itemView = layoutInflater.inflate(R.layout.debug_tool_item, parent, false)
